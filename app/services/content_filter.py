...
_FILTER_PATTERNS = [
    ...
    (
        re.compile(
            r"\b(how\s+to\s+)?(make|build|create|synthesize|manufacture)\s+(a\s+)?(bomb|explosive|weapon|gun|poison|drug|narcotic)\b"
        ),
        "Content blocked: harmful instructions",
    ),
    (re.compile(r"\b(self[\s-]?(harm|hurt|kill|destruct)|suicide|cutting)\b"), "Content blocked: self-harm content"),
    (re.compile(r"\b(fuck|shit|asshole|bitch|cunt|motherfucker)\b"), "Content blocked: profanity"),
    (
    ...
    (re.compile(r"\bexplicit\s+(sexual|adult|nude|porn|xxx)\b"), "Content blocked: explicit content"),
    (
