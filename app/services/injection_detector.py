...
_INJECTION_PATTERNS = [
    ...
    (re.compile(r"\bdan\b|do\s+anything\s+now|jail\s*break"), "Prompt injection detected: jailbreak attempt"),
    (re.compile(r"output\s+(in\s+)?base64|encoded\s+in\s+base64"), "Prompt injection detected: encoding bypass"),
    (
        re.compile(r"(forget|ignore|discard)\s+(everything|all|your|previous)"),
        "Prompt injection detected: instruction override",
    ),
    (re.compile(r"you\s+(have\s+been|are\s+)\s*hacked"), "Prompt injection detected: system compromise claim"),
    (
    ...
    (re.compile(r"\[\s*system\s*\]|<\|im_start\|>|<\|im_end\|>"), "Prompt injection detected: token injection"),
    (
